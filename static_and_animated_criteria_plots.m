...
for iii=1:length(list.models)
    ...
    m(6)=m(1);
    m(6).name = 'optRDM';
    %model_names = {'Optimal', 'Opt. Asym.', 'Fixed', 'Lin.', 'Quad.'}; %
    %this is just for marking the text
    sig1=3;
    sig2=12;
    mean_symcat = 5; % RDM symmetrical category mean.
    ...
    sss=1:nSteps;
    %extra = 1:nSteps%1:round(2*nSteps/3);
        %sss = [1:nSteps extra];
        %sat = 2*length(extra)/3;
        %alp=15;
        %slowdownframe = nSteps + round(length(extra)/2);
    ...
    iii
    models = list.models(iii);
    extraplot=list.extraplot{iii};
    xp_region_fill = list.xp_region_fill(iii);
    dp_region_fill = list.dp_region_fill(iii);
    
    % i think this loop is unnecessary. just change the above line from models to model_id?
    for model_id = models
        p = m(model_id).extracted(subject).best_params;
        model = m(model_id).name;
        p = parameter_variable_namer(p,strrep(model,' ','_'));
...
for modelno = 1 : nModels
    for subject = subjects
        ...
        maxsig = sqrt(p(3)+p(1)*min(true_contrasts).^-p(2));
        minsig = sqrt(p(3)+p(1)*max(true_contrasts).^-p(2));
        
        switch space
            case 'sig'
                sigs = linspace(minsig-1, maxsig+1, nSteps);
                x=sigs;
            case 'logcontrast'
        ...
        if any(regexp(gen.opt(modelno).name, 'asym'))
            d_bounds = flipud(p(4:10));
        elseif any(regexp(gen.opt(modelno).name, 'opt'))
            d_bounds = [flipud(p(5:7)); log(p(4)/(1-p(4))); -p(5:7)];
        end
        ...
        elseif any(regexp(gen.opt(modelno).name, 'fixed'))
            criteria=[zeros(1,nSteps);
                repmat(p(4:10),1,nSteps);
                80*ones(1,nSteps)];
        elseif any(regexp(gen.opt(modelno).name, '^lin2'))
            criteria=[zeros(1,nSteps);
                repmat(p(4:10),1,nSteps)+p(11:17)*sigs;
                80*ones(1,nSteps)];
        elseif any(regexp(gen.opt(modelno).name, '^quad2'))
            criteria=[zeros(1,nSteps);
                repmat(p(4:10),1,nSteps)+p(11:17)*sigs.^2;
                80*ones(1,nSteps)];
        elseif any(regexp(gen.opt(modelno).name, '^quad3'))
            criteria=[zeros(1,nSteps);
                repmat(p(4:10),1,nSteps)+p(11:17)*sigs.^2+p(18:24)*sigs;
                80*ones(1,nSteps)];
        end
        ...
        for i = 1:8
            f = fill([x fliplr(x)], [criteria(i,:) fliplr(criteria(i+1,:))],colors(i,:));
            hold on
       ...
       % if subject == 1
            ylabel({[gen.opt(modelno).name(1:10) '...'];'|x| (°)'})
        %end
    end
end
