...
for f = 1:length(files)
    t = regexp(files{f},'\.(.*?)\.','tokens');
    newjob(f) = str2num(t{1}{1});
end
[~,sort_idx]=sort(newjob);
job_files = files(sort_idx)
for f = [1 18 36 54] %55:68%length(job_files)
    load(job_files{f})
    hpc=0;
...
for m = 1:length(model)
    for d = 1:length(m(model).extracted)
...
for i = 1:5
subplot(1,5,i)
xxx=hist(stA.data(i).raw.resp,8)
bar(1:8,xxx)
end
