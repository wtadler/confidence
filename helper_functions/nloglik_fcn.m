...
try
    ...
    elseif strcmp(model.family, 'opt') && model.diff_mean_same_std
        k = (2*(bf(0)+d_noise).*(repmat(sig, nDNoiseSets, 1).^2 + category_params.sigma_s^2) - category_params.mu_2^2 + category_params.mu_1^2)...
            / (2*(category_params.mu_1 - category_params.mu_2)); % ADAPT FOR D NOISE
    ...
    elseif strcmp(model.family, 'MAP') && ~model.diff_mean_same_std % figure this out for diff_mean_same_std
        %     zoomgrid = false;
        %     if zoomgrid
        %         dx_fine = .5;
        ...
        for i = 1:len
            ...
            shat_lookup_table(i,:) = gmm1max_n2_fast([normpdf(xVec,0,sqrt(sig1^2 + cur_sig^2)) normpdf(xVec,0,sqrt(sig2^2 + cur_sig^2))],...
                [mu1 mu2], repmat([ksq1(i) ksq2(i)],xSteps,1),niter);
...
%                 sqrtk2 = sqrt(k2);
%                 sqrtk2 = sqrtk2(raw.contrast_id); % this should provide a speedup but doesn't
                
%                 a = sqrt(repmat(k1 - bf((raw.Chat - 1)./2 - raw.Chat .* raw.g), nDNoiseSets, 1) + d_noise_big) ./ repmat(sqrt(k2), nDNoiseSets, 1);
                a = sqrt(bsxfun(@rdivide,bsxfun(@minus, bsxfun(@plus, k1, d_noise_draws'), bf((raw.Chat - 1)./2 - raw.Chat .* raw.g)), k2));

%                 b = sqrt(repmat(k1 - bf((raw.Chat + 1)./2 - raw.Chat .* raw.g), nDNoiseSets, 1) + d_noise_big) ./ repmat(sqrt(k2), nDNoiseSets, 1);
                b = sqrt(bsxfun(@rdivide,bsxfun(@minus, bsxfun(@plus, k1, d_noise_draws'), bf((raw.Chat + 1)./2 - raw.Chat .* raw.g)), k2));
        ...
        elseif model.diff_mean_same_std
            ...
            p_conf_choice = fb - fa; % equivalent to 0.5 * (erf((b + raw.s)./(sqrt(2)*sig)) - erf((a + raw.s)./(sqrt(2)*sig)));
        end
